...
async function showMainApp() {
    ...
    socket.on('request_updated', (updatedRequest) => {
        ...
        if (updatedRequest.driverId === currentUser.id || 
            ...
            loadPanelData();
            updateAllNotifications();
            refreshAllMapMarkers();  // اضافه برای بروزرسانی نقشه
            if (currentUser.role === 'driver') {
                loadDriverStatus();
                loadDriverActiveMission();
            } else if (currentUser.role === 'greenhouse') {
    ...
    socket.on('connection_updated', (updatedConnection) => {
        console.log('Received connection_updated event:', updatedConnection);
        // بروزرسانی اتصالات محلی
        ...
        // اگر اتصال مربوط به کاربر فعلی باشه، UI رو بروز کن
        if (updatedConnection.sourceId === currentUser.id || updatedConnection.targetId === currentUser.id) {
            loadPanelData();
            updateAllNotifications();
            if (currentUser.role === 'driver') {
            ...
            return `
                <div class="border rounded-lg p-3 flex justify-between items-center ${isSuspended ? 'bg-yellow-50' : 'bg-blue-50'}">
                    <div>
                        ...
                        ${isSuspended ? '<p class="text-xs text-yellow-700 font-bold">(معلق)</p>' : ''}
                    </div>
                    <div class="flex items-center space-x-2 space-x-reverse">
...
async function completeLoadingProcess() {
    ...
    const response = await api.createConsolidatedDelivery(missionsToConsolidate);
...
document.addEventListener('visibilitychange', async () => {
    if (document.visibilityState === 'visible') {
        ...
        // Note: Removed misplaced storage event check (event.key) as this is visibilitychange, not storage
        console.log("Reloading data and refreshing UI.");
        await loadDataFromServer();
        loadPanelData();
